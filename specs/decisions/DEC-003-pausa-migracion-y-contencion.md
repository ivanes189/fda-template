...
**Enmendada el 2026-09-13 por [`DEC-008`](DEC-008-cierre-y-division-de-wp-009.md):** §§2, 4 y 6. WP-009 se cierra bloqueado tras agotar 2/2 ciclos, `ACTIVE` vuelve a reposo y el trabajo restante se divide entre los futuros WP-013 y WP-014. La enmienda entra en vigor en el mismo diff atómico de once archivos que crea DEC-008.
...
| 2 | `WP-009` | Cierre de operador: registra el bloqueo tras 2/2 ciclos, preserva la candidata C2, marca el contrato `blocked` y escribe reposo en `ACTIVE`, todo en el diff atómico de `DEC-008` | WP-009 cerrado como bloqueado y `ACTIVE` en reposo |
| 2.a | `WP-013` | Cierre de operador: registra el bloqueo tras C3 excepcional, preserva la candidata, marca el contrato `blocked` y escribe reposo en `ACTIVE`, todo en el diff atómico de recuperación de `DEC-008` | WP-013 cerrado como bloqueado y `ACTIVE` en reposo |
| 2.b | **Reposo** | **Completado:** acto de operador que creó y aprobó el contrato de `WP-014`, limitado a exactamente diez pins por SHA y a un parche Git exacto aplicado humanamente | **Cumplida:** contrato de `WP-014` aprobado y activado por acto separado |
| 2.c | `WP-014` | **Completado:** el agente preparó los mismos bytes para `git apply --check --index -` y `git apply --index -`; una persona los aplicó por `stdin` en un worktree limpio y dedicado; se comprobó el diff exacto, se probó, revisó y fusionó | **Cumplida por PR #40 y este cierre:** WP-014 fusionado y `ACTIVE` devuelto a reposo |
...
| 5 | **Reposo** | **PARADA TÉCNICA.** DEC-011 eligió el rumbo y DEC-012 resolvió la gramática, pero la candidata agotada de WP-015 sigue preservada y no conforme | Autorización separada para una candidata replanteada; después, actos separados de aprobación, admisión y activación de `WP-015` |
...
1. Una autorización humana futura y separada podrá preparar una candidata
   replanteada de `WP-015`, sucesor limpio de WP-002, conforme a DEC-012. Otra
   autorización deberá crear y aprobar el contrato, admitirlo y activarlo.
   Hasta entonces el identificador está solo reservado.
...
| `WP-008` | Cerrado `blocked` tras C5 excepcional conforme a `DEC-009`; candidata D6-A preservada y no importada, A/B final no ejecutado y transición solidaria a reposo |
| `WP-009` | Cierre bloqueado, preservación de su candidata C2 y transición solidaria a reposo conforme a `DEC-008` |
...
**Admisión atómica de `DEC-008`, del cierre de WP-009 y de los identificadores sucesores.** Se incorporan modificando directamente esta lista en el mismo diff que crea `DEC-008`; la decisión no se autoautoriza. La composición es cerrada y consta exactamente de once archivos: `specs/decisions/DEC-008-cierre-y-division-de-wp-009.md`, esta `DEC-003`, `docs/03-hoja-de-ruta.md`, `docs/manual/05-bloqueos-y-parada.md`, `work-packages/WP-009-acciones-fijadas-por-sha.md`, `work-packages/ACTIVE`, `specs/finops/fx-rates.md` y los cuatro artefactos `evidence/WP-009/{CIERRE-BLOQUEADO.md,cost.md,coste-f1.md,MANIFIESTO-CANDIDATA-C2.md}`. Admitir los identificadores `WP-013` y `WP-014` fija la secuencia futura; **no crea sus contratos, no los activa y no autoriza su ejecución**.

**Admisión atómica del cierre de recuperación de WP-013.** Se modifica
directamente esta lista en el mismo diff; la enmienda no se autoautoriza. La
composición cerrada consta exactamente de doce archivos: `DEC-008`, esta
`DEC-003`, `docs/03-hoja-de-ruta.md`, `docs/manual/05-bloqueos-y-parada.md`,
`work-packages/WP-013-materializador-seguro-rutas-protegidas.md`,
`work-packages/ACTIVE` y los seis artefactos
`evidence/WP-013/{CIERRE-BLOQUEADO.md,MANIFIESTO-CANDIDATA-C3.md,cost.md,coste-f1.md,revision-astra.md,revision-astra-enfocada-c3.md}`.
Todos viajan juntos o ninguno. No se importa ningún archivo de implementación
de la candidata, no se crea o activa WP-014 y no se modifican workflows.

**Admisión atómica del cierre de WP-014.** El cumplimiento posterior de la
secuencia de DEC-008 se registra modificando directamente esta decisión en el
mismo diff de operador. La composición cerrada consta exactamente de siete
archivos: esta `DEC-003`, `docs/03-hoja-de-ruta.md`,
`docs/manual/05-bloqueos-y-parada.md`,
`specs/requirements/REQ-FDA-002-workflows-endurecidos.md`,
`work-packages/WP-014-diez-pins-acciones-sha.md`, `work-packages/ACTIVE` y
`evidence/WP-014/CIERRE.md`. Todos viajan juntos o ninguno. No se modifica
ningún workflow, DEC-008, ruleset, candidata histórica, rama o worktree; no se
crea, corrige, activa ni ejecuta otro WP.

**Admisión atómica de `DEC-009` y del cierre bloqueado de WP-008 D6-A.** Se
modifica directamente esta lista en el mismo diff; la decisión no se
autoautoriza. La composición cerrada consta exactamente de dieciocho archivos:
`DEC-009`, esta `DEC-003`, `docs/03-hoja-de-ruta.md`,
`docs/manual/05-bloqueos-y-parada.md`,
`work-packages/WP-008-runtime-fail-closed.md`, `work-packages/ACTIVE` y los doce
...
**Admisión atómica de `DEC-012` y de la gramática de alcance.** Se modifica
directamente esta lista en el mismo diff; la decisión no se autoautoriza. La
composición cerrada consta exactamente de ocho archivos: `DEC-012`, esta
`DEC-003`, `DEC-002`, `DEC-011`, `docs/03-hoja-de-ruta.md`,
`work-packages/_TEMPLATE.md` y
`docs/manual/{03-redactar-un-wp.md,05-bloqueos-y-parada.md}`. Todos viajan
juntos o ninguno. Es un acto normativo de operador con `ACTIVE` en reposo: no
crea, modifica, aprueba, admite ni activa WP-015; no cambia guard, scripts,
tests, workflows, ruleset, contratos existentes, evidencias o candidatas.
...
- [x] **Acciones fijadas por SHA.** `WP-014` fusionado por la PR #40 en
  `ea004919b661baf206257952f2fd5ca7bfad2c05`; el criterio de
  verificación n.º 2 de `REQ-FDA-002` devuelve vacío; el diff contiene
  exactamente los diez pins autorizados y el procedimiento Git exacto aplicado
  humanamente no tiene hallazgos ALTOS o CRÍTICOS abiertos en el entregable y
  procedimiento aplicado de WP-014. Ver `evidence/WP-014/CIERRE.md`. Los
  cierres bloqueados de WP-009 y WP-013 no satisfacen esta condición.
